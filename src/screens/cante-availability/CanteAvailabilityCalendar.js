    ...
    <div className="shadow-lg rounded-xl bg-white p-5 mt-2 mx-auto p-5 max-w-xl">
      {
        token ? (
          ...
          <PibPrimaryButton onClick={edit}>Minha disponibilidade</PibPrimaryButton>
        </div>
        ):(
        ...
        return (
          <div key={date} className="h-10 p-1 cursor-default group w-[14%]">
            <div className="flex items-center justify-center relative">
              <div
                className="group-hover:bg-gray-200 rounded-full w-8 h-8 relative overflow-visible flex items-center justify-center"
              >
                <div>
                  { date.getDate() }
                </div>
                ...
                {availabilitiesForDate.length >0 ? (  
                  <div
                    className="bg-green-400 rounded-full text-[8px] text-white w-3 h-3 absolute top-0 right-0"
                  >
                    <div className="flex items-center justify-center h-full">
                      <div>
                        { availabilitiesForDate.length }
                      </div>
                    </div>
                  </div> 
                ) : null}
                <PibAvailabilityDropdown date={date} availabilities={availabilitiesForDate}/>
              </div>
            </div>
          </div>
        )})}
